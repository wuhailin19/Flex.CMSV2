...
{
    public class MySqlSqlTableServices : ISqlTableServices
    {
        public string CreateContentTableSql(string TableName) => "CREATE TABLE " + TableName + "" +
                                     "(" +
                                      ...
                                      "`IsHide` BOOLEAN NOT NULL DEFAULT 0," +
                                      "`IsSilde` BOOLEAN NOT NULL DEFAULT 0," +
        ...
        public string AlertTableField(string TableName, string oldfiledName, string filedName, string filedtype) => $"ALTER TABLE `{TableName}` CHANGE COLUMN `{oldfiledName}` `{filedName}` {ConvertDataType(filedtype)};";

        public string ReNameTableField(string TableName, string oldfiledName, string filedName) => $"ALTER TABLE `{TableName}` CHANGE COLUMN `{oldfiledName}` `{filedName}`;";
