layui.define(['element'], function (exports) {
    ...
    function createStyle(ulClassName, width) {
        var style = '.{name} {position: absolute;width: 100px;z-index: 9999;display: none;box-shadow: 0 2px 4px rgb(0 0 0 / 12%);background-color: #fff;padding: 2px;color: #333;border: 1px solid #eee;border-radius: 2px;cursor: pointer;}.{name} li {text-align: center;display: block;line-height: 32px;}.{name} li:hover {background-color: rgb(102, 177, 255);color:#fff;transition: all 0.3s; -webkit-transition: all 0.3s}'
    ...
    function CreateRightMenu(rightMenuConfig) {
        ...
        var tmpHtml = '<ul class="rightmenu {className}">{liStr} </ul>'.format({ liStr: li, className: rightMenuConfig.filter })
    ...
    function _CustomRightClick(rightMenuConfig) {
        ...
        $("." + rightMenuConfig.filter).bind('mouseover', function () {
            clearTimeout(tio);
        })
        $("." + rightMenuConfig.filter).bind('mouseout', function () {
