...
namespace Flex.EFSql.Register
{
    ...
    public static class UnitOfWorkServiceExtensions
    {
        ...
        public static IServiceCollection AddUnitOfWorkService<TContext>(this IServiceCollection services, 
            System.Action<DbContextOptionsBuilder> action) where TContext : DbContext
        {
            //注册dbcontext
            services.AddDbContext<TContext>(action);
            //注册工作单元
            //services.AddScoped<IEfCoreIUnitWork, EfCoreUnitWork<TContext>>();
        ...
        /// <remarks>此方法仅支持一个db上下文，如果多次调用，将抛出异常。</remarks>
        /// <returns></returns>
        public static IServiceCollection AddShardingCoreUnitOfWorkService<TContext>(this IServiceCollection services,
            System.Action<DbContextOptionsBuilder> action) where TContext : AbstractShardingDbContext
        {
            //注册dbcontext
            services.AddDbContext<TContext>(action);
            //注册工作单元
