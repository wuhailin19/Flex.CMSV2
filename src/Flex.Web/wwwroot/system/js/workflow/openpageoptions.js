var defaultOptions = {
    ...
    setAddIframe: function (layer, insTb) {
        var self = this;
        let widthstr = self.addwidths == 0 ? '90%' : self.addwidths + 'px';
        let heightstr = self.addheights == 0 ? '90%' : self.addheights + 'px';
        //iframe窗
        layer.open({
            type: 2,
            title: self.addTitle,
            ...
            content: routePageLink + 'AddWorkFlow',
            end: function () {
    ...
    editIframe: function (layer, insTb) {
        var self = this;
        let widthstr = self.editwidths == 0 ? '90%' : self.editwidths + 'px';
        let heightstr = self.editheights == 0 ? '90%' : self.editheights + 'px';
        ...
        layer.open({
            ...
            maxmin: true, //开启最大化最小化按钮
            area: [widthstr, heightstr],
            content: routePageLink + 'DesignFlowchart',
            end: function () {
                if (isreload) {
                    defaultOptions.callBack(insTb);
                }
