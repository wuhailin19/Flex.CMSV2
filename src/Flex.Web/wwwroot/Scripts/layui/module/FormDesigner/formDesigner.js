...
layui.config({ base: '/Scripts/layui/module/formdesigner/' }).define(["layer", 'flow', "laytpl", "element", "form", "slider", "laydate", "rate", "colorpicker", "carousel", "upload", "formField", "staticField", "numberInput", "iconHhysFa", "cron", "labelGeneration"]
    , function (exports) {
        ...
        function generateUUID() {
            ...
            if (crypto) {
                ...
                return (
                    ...
                    buffer[5].toString(16) + buffer[6].toString(16) + buffer[7].toString(16)
                );
            } else {
                return "-1";
            }
            ...
            for (var key in json) {
                ...
                switch (key) {
                    ...
                    case 'LocalSource':
                        ...
                        _html += '  <label class="layui-form-label">{0}</label>'.format(staticField.lang[key]);
                        _html += '  <div class="layui-input-block">';
                        _html += '     <input type="checkbox" name="{0}" {1} lay-skin="switch" lay-text="本地|远程">'
                            .format(key, json[key] ? 'checked' : '');
                        _html += '  </div>';
                        _html += '</div>';
                        break;
                    case 'remoteUrl':
                        ...
                        _html += '  <label class="layui-form-label">{0}</label>'.format(staticField.lang[key]);
                        _html += '  <div class="layui-input-block">';
                        _html += '   <input type="text" name="{0}-value"  autocomplete="off" value="{1}" class="layui-input">'.format(json.tag, json[key]);
                        _html += '  </div>';
                        _html += '</div>';
                        break;
                    case 'options':
                        if (json.tag == 'checkbox') {
                            if (!json.LocalSource) {
                                ...
                                _html += '  <div class="layui-input-block">';
                                _html += '  <div class="layui-inline" style="margin-right: 0px;width:110px;">';
                                _html += '    <button style="margin: 5px 0px 0px 5px;" type="button" id="select-option-remote" class="layui-btn layui-btn-primary layui-btn-sm">获取数据</button>'
                                _html += '  </div>';
                                _html += '  </div>';
                                _html += '  </div>';
                                break;
                ...
                switch (_key) {
                    ...
                    case 'checkbox-value':
                        if (_json.tag == "checkbox") {
                            if (!_json.LocalSource) {
                                let remoteurl = _value;
                                _json.remoteUrl = remoteurl;
                                break;
                            }
                        }
        ...
        Class.prototype.render = function () {
            ...
            $('#initformproperty').on('click', function () {
                options.data = json_data.slice();
                that.renderForm();
            });
            //注册导出数据
